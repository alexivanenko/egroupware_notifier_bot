...
// Connect uses client_secret.json file for connection to GMail Service
func Connect(ctx context.Context) (*gmail.Service, error) {
	if service == nil {
		b, err := ioutil.ReadFile(config.GetRootDir() + "/client_secret.json")
		if err != nil {
		...
		client := getClient(ctx, gMailConfig)
